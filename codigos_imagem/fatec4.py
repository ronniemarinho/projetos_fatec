...
#AGE_INTERVALS = ['(0, 3)', '(4, 7)', '(8, 14)', '(15, 24)',
...
def predict_age(input_path: str):
    ...
    imagem_sobreposicao = cv2.imread("adamantina.png",
                                     cv2.IMREAD_UNCHANGED)  # Substitua pelo caminho da sua imagem de sobreposição

    imagem_sobreposicao = cv2.resize(imagem_sobreposicao, (220, 150))

    # Defina a posição onde a imagem de sobreposição será inserida na imagem maior
    posicao_x = 0  # Substitua pela posição x desejada
    posicao_y = 0  # Substitua pela posição y desejada
    ...
    y1, y2 = max(posicao_y, 0), min(posicao_y + altura_sobreposicao, img.shape[0])
    x1, x2 = max(posicao_x, 0), min(posicao_x + largura_sobreposicao, img.shape[1])
    ...
    img[y1:y2, x1:x2][canal_alpha > 0] = canais_rgb[canal_alpha > 0]
    ...
    for i, (start_x, start_y, end_x, end_y) in enumerate(faces):
        ...
        print("="*30, f"Face {i+1} Predição Probabilidade", "="*30)
        ...
        import re
        texto = age
        # Use uma expressão regular para encontrar os números dentro dos parênteses
        padrao = r'\((\d+), (\d+)\)'
        match = re.search(padrao, texto)

        numeros = match.groups() if match else (None, None)
        primeiro_numero = int(numeros[0]) if numeros[0] is not None else None
        segundo_numero = int(numeros[1]) if numeros[1] is not None else None
        ...
        numero_aleatorio = random.randint(primeiro_numero - 4, segundo_numero + 5)
        label = f"Idade real entre {primeiro_numero} a {segundo_numero} anos."
        #label = f"Você possui entre:{age} - {age_confidence_score*100:.2f}%"
    ...
    display_thread = threading.Thread(target=display_img, args=('IA Fatec Adamantina', frame))
    display_thread.start()
    # Display processed image
    #display_img('IA Fatec Adamantina', frame)
    ...
    texto = f"Você possui entre {primeiro_numero} e {segundo_numero} anos, com rostinho de {numero_aleatorio} anos"
    ...
    if primeiro_numero == 15 and segundo_numero == 20:
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if ret:
            # Adicione o quadro ao cache
            frame_cache.append(frame)

            # Mantenha o cache com no máximo max_cache_frames quadros
    ...
    # Defina o caminho da imagem diretamente no código
